...
class SMSService:
    ...
    @staticmethod
    async def _send_to_provider(phone: str, message: str, otp: str = None):
        """
        Internal router that sends the message.
        Supports Mocking in DEBUG mode.
        Returns the Session ID (string) on success, or None on failure.
        """
        ...
        if getattr(settings, "DEBUG", False):
            ...
            logger.info(f"-----------------")
            return "debug_session_id_12345"

        # 2. Production: Route to 2Factor.in
        return await SMSService._send_2factor(phone, otp)

    # --- Provider Integrations ---

    @staticmethod
    async def _send_2factor(phone: str, otp: str):
        ...
        template_name = getattr(settings, "TWO_FACTOR_TEMPLATE_ID", "RojiRoti_OTP")
        
        # Build the 2Factor URL
        url = f"https://2factor.in/API/V1/{api_key}/SMS/{clean_phone}/{otp}/{template_name}"
        
        # --- LOGGING: SEE WHAT IS HAPPENING ---
        logger.info(f"🚀 Sending Request to 2Factor URL: {url}")
        
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(url)
                
                # --- LOGGING: SEE WHAT 2FACTOR SENT BACK ---
                logger.info(f"2Factor Response: {response.text}")
                
                data = response.json()

                if data.get("Status") == "Success":
                    # RETURN THE SESSION ID INSTEAD OF JUST 'True'
    ...
    @staticmethod
    async def send_subscription_reminder(phone: str, days_left: int):
