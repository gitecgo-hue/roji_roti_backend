...
class IVRService:
    @staticmethod
    async def trigger_click_to_call(employer_phone: str, employee_phone: str) -> bool:
        """
        Triggers a Call Masking bridge.
        1. The IVR calls the Employer.
        2. When Employer picks up, it calls the Employee.
        3. Bridges them together. Neither sees the other's real number.
        """
        if getattr(settings, "DEBUG", False):
            logger.info(f"📞 [MOCK IVR] Bridging Employer {employer_phone} with Employee {employee_phone}")
            return True
    ...
    @staticmethod
    async def trigger_automated_voice_blast(employee_phone: str, audio_file_id: str) -> bool:
        """
        Example: Call an employee and play an automated pre-recorded message.
